...
class API
{
     ...
     *
     * @return int (signifcant digits) based on the minimum order amount
     */
    ...
    {
        $params = ['sapi' => true];
        if ('' !== $symbol && 'string' === gettype($symbol)) {
            $params['symbol'] = $symbol;
        }
    ...
    public function withdraw(string $asset, string $address, $amount, $addressTag = null, $addressName = '', bool $transactionFeeFlag = false, $network = null, $orderId = null)
    {
        ...
        if (false === is_null($addressName) && false === empty($addressName)) {
            $options['name'] = str_replace(' ', '%20', $addressName);
        }
        if (false === is_null($addressTag) && false === empty($addressTag)) {
            $options['addressTag'] = $addressTag;
        }
        if (false === is_null($network) && false === empty($network)) {
            $options['network'] = $network;
        }
        if (false === is_null($orderId) && false === empty($orderId)) {
     ...
     *
     * @see https://github.com/binance/binance-spot-api-docs/blob/master/rest-api.md#old-trade-lookup-market_data
    ...
    {
        if (false === is_int($limit)) {
            $limit = 100;
        }

        if (false === isset($symbol) || false === is_string($symbol)) {
            // WPCS: XSS OK.
        ...
        if (false === isset($this->info[$symbol])) {
            $this->info[$symbol] = [];
        }
    ...
    {
        if (false === is_array($priceData)) {
            $priceData = false;
        }

        $account = $this->httpRequest('v3/account', 'GET', [], true);

        if (false === is_array($account)) {
            echo 'Error: unable to fetch your account details'.PHP_EOL;
        }

        if (false === isset($account['balances']) || empty($account['balances'])) {
    ...
    public function order(string $side, string $symbol, $quantity, $price, string $type = 'LIMIT', array $flags = [], bool $test = false)
    {
        ...
        if (false === is_string($price)) {
            // WPCS: XSS OK.
            echo 'warning: price expected string got '.gettype($price).PHP_EOL;
        }

        if ('LIMIT' === $type || 'STOP_LOSS_LIMIT' === $type || 'TAKE_PROFIT_LIMIT' === $type) {
            $opt['price']       = $price;
            $opt['timeInForce'] = 'GTC';
        }

        if ('MARKET' === $type && isset($flags['isQuoteOrder']) && $flags['isQuoteOrder']) {
            unset($opt['quantity']);
        ...
        $qstring = (false === $test) ? 'v3/order' : 'v3/order/test';
    ...
    public function depthCache($symbols, callable $callback)
    {
        ...
        foreach ($symbols as $symbol) {
            ...
            $connector($this->getWsEndpoint().strtolower($symbol).'@depth')->then(function ($ws) use ($callback, $symbol, $loop, $endpoint) {
                $ws->on('message', function ($data) use ($callback, $loop, $endpoint) {
                    if (false === $this->subscriptions[$endpoint]) {
                    ...
                    $symbol = $json['s'];
                    if (0 === intval($this->info[$symbol]['firstUpdate'])) {
    ...
    public function chart($symbols, string $interval = '30m', callable $callback = null, $limit = 500)
    {
        ...
        foreach ($symbols as $symbol) {
            ...
            $connector($this->getWsEndpoint().$endpoint)->then(function ($ws) use ($callback, $symbol, $loop, $endpoint, $interval) {
                $ws->on('message', function ($data) use ($loop, $callback, $endpoint) {
                    if (false === $this->subscriptions[$endpoint]) {
                        // $this->subscriptions[$endpoint] = null;
                        $loop->stop();
    ...
    public function userData(&$balance_callback, &$execution_callback = false)
    {
        ...
        $connector($this->getWsEndpoint().$this->listenKey)->then(function ($ws) {
            $ws->on('message', function ($data) use ($ws) {
                if (false === $this->subscriptions['@userdata']) {
                    // $this->subscriptions[$endpoint] = null;
                    $ws->close();
    ...
    */
    ...
    protected function setupApiConfigFromFile(string $file = null)
    {
        ...
        if (false === empty($this->api_key) || false === empty($this->api_secret)) {
            return;
        }
    ...
    protected function setupProxyConfigFromFile(string $file = null)
    {
        ...
        if (false === is_null($this->proxyConf)) {
            return;
        }
        if (false === file_exists($file)) {
            echo 'Unable to load config from: '.$file.PHP_EOL;
        ...
        $contents = json_decode(file_get_contents($file), true);
        if (false === isset($contents['proto'])) {
            return;
        }
        if (false === isset($contents['address'])) {
            return;
        }
        if (false === isset($contents['port'])) {
            return;
        }
        ...
        if (isset($contents['user'])) {
            $this->proxyConf['user'] = $contents['user'] ?? '';
        }
        if (isset($contents['pass'])) {
            $this->proxyConf['pass'] = $contents['pass'] ?? '';
        }
    }
    ...
    protected function httpRequest(string $url, string $method = 'GET', array $params = [], bool $signed = false)
    {
        ...
        if (false === $this->caOverride) {
            if (false === file_exists(getcwd().'/ca.pem')) {
        ...
        if (isset($json['msg']) && !empty($json['msg'])) {
            if ('v1/system/status' !== $url && 'v3/systemStatus.html' !== $url && 'v3/accountStatus.html' !== $url) {
                // should always output error, not only on httpdebug
                // not outputing errors, hides it from users and ends up with tickets on github
    ...
    protected function balanceData(array $array, $priceData)
    {
        ...
        foreach ($array['balances'] as $obj) {
            ...
            if ('BTC' === $asset) {
                ...
                continue;
            } elseif ('USDT' === $asset || 'USDC' === $asset || 'PAX' === $asset || 'BUSD' === $asset) {
            ...
            } elseif (false === isset($priceData[$symbol])) {
                $btcValue = $btcTotal = 0;
            } else {
    ...
    protected function downloadCurlCaBundle()
    {
        ...
        if (false === is_writable(getcwd())) {
            exit(getcwd().' folder is not writeable, please check your permissions to download CA Certificates, or use $api->caOverride = true;');
        }
